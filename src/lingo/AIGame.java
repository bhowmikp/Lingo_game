 ...
 * @author prantar
 */
public class AIGame extends JFrame implements KeyListener, Runnable {
    
    JPanel contentPane;
    Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
    int x = screenSize.width, y = screenSize.height;      // contentPane
    int speed = 25;             // speed of game
    Thread thread; 
    boolean start=true;
    boolean start1=true;
    ...
    // location of light bikes to give computer brain
    int xLightBike1, xLightBike2, yLightBike1, yLightBike2;
    
    // brain booleans
    boolean up= false,down=false, left=false, right=true;
    
    int directionComputerHorizontal, directionComputerVertical;
    int[] ch= new int[20];    
    int item,point;
    

    public AIGame(String name1, String name2, String colour1, String colour2, String sound,int point,int item,int itemch[]){
        super("TRON");
         this.point= point;
        this.item=item;
        ch=itemch;
    ...
    public void brain(){
        for (int i = 1; i < bikeLength2; i++) {
            xLightBike1=player1[i].getX(); // computer
            xLightBike2=player2[i].getX();
            yLightBike1=player1[i].getY(); // computer
            yLightBike2=player2[i].getY();
            
            if (((player1[0].getX()) > ((player2[0].getX())-10 )) && ((player1[0].getY())==(player2[0].getY()))) {  // Head on 
                moveDirectionVertical();
                moveForward1();
            }
            
            
            if (((player1[0].getX())==xLightBike1) && (player1[0].getY() == (yLightBike1-10)) && (down==true)) {   // computer self inflict from above
                moveDirectionHorizontal();
                moveForward1();
            }
            
            if (((player1[0].getX())==xLightBike1) && (player1[0].getY() == (yLightBike1+10)) && (up==true)) {   // computer self inflict from down
                moveDirectionHorizontal();
                moveForward1();
            }
            
            if (((player1[0].getX())==xLightBike1-10) && (player1[0].getY() == (yLightBike1)) && (right==true)) {   // computer self inflict from right
                moveDirectionVertical();
                moveForward1(); 
            }
            
            if (((player1[0].getX())==xLightBike1+10) && (player1[0].getY() == (yLightBike1)) && (left==true)) {   // computer self inflict from left
                moveDirectionVertical();
                moveForward1();  
            }
            
                        if (((player1[0].getX())==xLightBike2) && (player1[0].getY() == (yLightBike2-10)) && (down==true)) {   // computer inflict human from above
                moveDirectionHorizontal();
                moveForward1();  
            }
            
            if (((player1[0].getX())==xLightBike2) && (player1[0].getY() == (yLightBike2+10)) && (up==true)) {   // computer inflict human from down
                moveDirectionHorizontal();
                moveForward1(); 
            }
            
            if (((player1[0].getX())==xLightBike2-10) && (player1[0].getY() == (yLightBike2)) && (right==true)) {   // computer inflict human from left
                moveDirectionVertical();
                moveForward1();  
            }
            
            if (((player1[0].getX())==xLightBike2+10) && (player1[0].getY() == (yLightBike2)) && (left==true)) {   // computerinflict human from right
                moveDirectionVertical();
                moveForward1();   
            }
            
            if ((lbx1[0] <= 0+10)) {      // left
                directionComputerVertical= (int) (Math.round(Math.random()* 2));
                switch (directionComputerVertical){
                    case 1:
                        up();
                        break;
                    case 0:
            ...
            if ((lby1[0] <= 0+10)) {      // up
                directionComputerHorizontal= (int) (Math.round(Math.random()* 1));
                switch (directionComputerHorizontal){
                    case 0:
                    case 2:
                        left();
                        break;
    ...
    public void moveDirectionHorizontal(){
        directionComputerHorizontal= (int) (Math.round(Math.random()* 1));
        switch (directionComputerHorizontal){
            case 0:     // left
                if (runLeft1 == true) {
    ...
    public void moveDirectionVertical(){
        directionComputerVertical= (int) (Math.round(Math.random()* 1));
        switch (directionComputerVertical){
            case 0: // up
                if (runUp1 == true) {
                    direction1x = 0;
    ...
    public void collision(){
        for (int i = 1; i < bikeLength2; i++) {
            if (player1[0].getLocation().equals( player1[i].getLocation() )) {// Self inflict 1st player
                JOptionPane.showMessageDialog(null, "COMPUTER LOSE (HIT HIMSELF)");
            ...
            if ((lbx1[0] <= 0)||(lbx1[0] >= x)||(lby1[0] <= 0)||(lby1[0] >= y)) {      // player 1 boundaries
                JOptionPane.showMessageDialog(null, "COMPUTER LOSE (HIT BOUNDARY)");
                Tron run = new Tron (name1, colour1, name2, colour2, sound,point,item,ch);
                dispose();
           ...
           if (player1[0].getLocation().equals( player2[0].getLocation() )) {  // Head on 
            ...
            if (player1[0].getLocation().equals( player2[i].getLocation() )) {// 1st player touch second player
                JOptionPane.showMessageDialog(null, "COMPUTER LOSE (HIT "+ name2 +")");
                Tron run = new Tron (name1, colour1, name2, colour2, sound,point,item,ch);
                dispose();
            ...
            if (player2[0].getLocation().equals( player1[i].getLocation() )) {// Second player touch 1st player
                JOptionPane.showMessageDialog(null, name2+ " LOSE (HIT COMPUTER");
                Tron run = new Tron (name1, colour1, name2, colour2, sound,point,item,ch);
                dispose();
